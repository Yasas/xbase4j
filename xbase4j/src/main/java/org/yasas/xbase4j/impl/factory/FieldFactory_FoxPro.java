...
public class FieldFactory_FoxPro extends FieldFactory_Common {
  private static FieldFactory instance = null;

  public static synchronized FieldFactory getInstance() {
    if (instance == null) {
      instance = new FieldFactory_FoxPro();
    }
  ...
  public Field<?> create(String name, char type) {
    switch (type) {
      case 'B' : return new DoubleField(name, type);
      case 'I' : return new IntegerField(name, type);
      case 'T' : return new DateTimeField(name, type);

      default : return super.create(name, type);
    }
  }
}

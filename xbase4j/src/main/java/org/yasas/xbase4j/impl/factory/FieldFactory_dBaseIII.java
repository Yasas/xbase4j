...
public class FieldFactory_dBaseIII extends FieldFactory_Common {
  private static FieldFactory instance = null;

  public static synchronized FieldFactory getInstance() {
    if (instance == null) {
      instance = new FieldFactory_dBaseIII();
    }
  ...
  public Field<?> create(String name, char type) {
    switch (type) {
      case 'M' : return new MemoField_dBaseIII(name, type);

      default : return FieldFactory_Common.getInstance().create(name, type);
    }
  }
}
